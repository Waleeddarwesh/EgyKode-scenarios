...
# The state bucket is created outside Terraform and versioned, because step 3
# restores a deleted state file from a previous version.
awslocal() { aws --endpoint-url=http://localhost:4566 "$@"; }
awslocal s3api create-bucket --bucket egykode-state-recovery >/dev/null 2>&1
awslocal s3api put-bucket-versioning --bucket egykode-state-recovery   --versioning-configuration Status=Enabled >/dev/null 2>&1
